...
## Then I read all files in R with the read table funktion too take a first look, f.e. with head() or tail()
...
## fist we have to combine the test and the training data to a complete data set
...
## The Readme further say that : Each feature vector is a row on the text file, so I took the vectors as row names:
...
## The ReadME file axplains that the y_files contain the lables and subect_files the subjects 
...
msX <- X[,c(mT,sT)] ## only return the columns that contain std or mean
...
## now i want 30 diffrent martixes for every subject
SYXs <- split(SYXmelt,SYXmelt[,1]) ## splits the data by subject
SYXclean <- lapply(SYXs, dcast, activities ~variable, mean)


write.table(SYXclean,file=".CourseProjectTextFile.txt", row.name=FALSE)
